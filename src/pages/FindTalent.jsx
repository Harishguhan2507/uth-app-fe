...
function FindTalent() {
  ...
  const filtered = useMemo(() => {
    return employees.filter((employee) => {
      const searchable =
        `${employee.name} ${employee.role} ${employee.skills.join(" ")}`.toLowerCase();
      ...
      const availOk =
        ...
        (availability === "part-time" && employee.availability === "Part-time");
      const expOk =
        level === "All" ||
              ...
              {skillOptions.map((skill) => (
                ...
                >
                  <input
                    ...
                    onChange={() =>
                      toggleValue(skill, selectedSkills, setSelectedSkills)
                    }
                  />
                  {skill}
                </label>
        ...
        <section>
          ...
          {showReason ? (
            <div className="mt-3 rounded-xl border border-indigo-200 bg-indigo-50 p-3 text-sm text-indigo-900">
              <span className="font-semibold">AI reason:</span> Strong React +
              Python combo with 2 previous ML dashboard projects. Currently
              available full-time.
            </div>
          ) : null}
          ...
          <div className="mt-4 space-y-3">
            {filtered.length > 0 ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-300 bg-white p-6 text-sm text-slate-500">
                No matches found. Try clearing filters or using broader search
                terms like `React`, `ML`, or `Python`.
              </div>
            )}
          </div>
        </section>
