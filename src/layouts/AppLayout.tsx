...
export const AppLayout = () => {
  ...
  const [sidebarCompact, setSidebarCompact] = useState(
    () => localStorage.getItem("uth_sidebar_compact") === "1",
  );
    ...
    <div className="min-h-screen bg-[hsl(var(--background))] text-[hsl(var(--foreground))]">
        ...
        {mobileOpen ? (
          ...
          >
            <motion.aside
              ...
              className="h-full w-[280px] border-r border-[hsl(var(--border))] bg-[hsl(var(--card))] p-3 shadow-[var(--shadow-floating)]"
      ...
      {/* <div className="mx-auto flex max-w-[1560px] gap-4 p-3">
        <AppSidebar
          ...
          onToggleCompact={() => setSidebarCompact((s) => !s)}
          session={session}
          onLogout={handleLogout}
        />

        <main className="min-w-0 flex-1">
          <header className="mb-4 flex items-center gap-3 rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-4 py-3 shadow-[var(--shadow-soft)] backdrop-blur">
            <button
            ...
            <div className="hidden md:block">
              <p className="text-[28px] font-semibold leading-7">
                AI Talent Matching
              </p>
              <p className="text-[10px] tracking-[0.35em] text-[hsl(var(--muted-foreground))]">
                WORKSPACE
              </p>
            </div>
            ...
            <button
              className="hidden items-center gap-1 rounded-lg border border-[hsl(var(--border))] px-2 py-1 text-xs text-[hsl(var(--muted-foreground))] sm:flex"
              onClick={() => setOpen(true)}
            >
              <Command size={12} />K
            </button>
            <button className="rounded-lg border border-[hsl(var(--border))] p-2">
              <Bell size={16} />
            </button>
            <ThemeToggle />
          </header>
